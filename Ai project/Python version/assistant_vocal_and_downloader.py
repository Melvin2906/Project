...
# Download function
def telecharger_video(url, dossier_sortie="C:/Users/Utilisateur/Videos"):
    options = {
...
def execute_command():
    ...
    download = ["télécharge", "download", "ajoute", "add"]
    verif, its = in_word(order, cmd)
    verif2 = in_word(order2, cmd)
    down, load = in_word(download, cmd)
    sites = {
    ...
    if verif == 1 and down != 1:
        ...
        song_name = cmd.replace(load, "").strip()
        speak(f"Téléchargement de {song_name} sur Youtube.")
        ...
        if results:
            ...
            print("Téléchargement terminé. Merci pour votre confiance")
